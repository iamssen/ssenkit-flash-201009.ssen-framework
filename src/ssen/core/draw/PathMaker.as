...
/**
 * GraphicsPath 를 만들어준다
 * @author ssen (i@ssen.name)
 */
public class PathMaker {
	private static const ARROW_RECT_COMMAND : Vector.<int> = Vector.<int>([1,2,2,2,2,2]);

	public static function arrowRect(direction : int, arrowSize : int, x : Number, y : Number, width : Number, height : Number, path : GraphicsPath = null) : GraphicsPath {
		if (!path) path = new GraphicsPath();
		path.commands = ARROW_RECT_COMMAND;
		var rect : Vector.<Number> = Vector.<Number>([x, y, x + width, y, x + width, y + height, x, y + height, x, y]);
		path.data = new Vector.<Number>();
		switch (direction) {
			case 0 :
				rect[1] = rect[3] = rect[9] = rect[1] + arrowSize;
				path.data = rect.slice(0, 2).concat(rect.slice(2), Vector.<Number>([x + (width >> 1), y]));
				break;
			case 1:
				rect[2] = rect[4] = rect[2] - arrowSize;
				path.data = rect.slice(0, 4).concat(rect.slice(4), Vector.<Number>([x + width, y + (height >> 1)]));
				break;
			case 2:
				rect[5] = rect[7] = rect[5] - arrowSize;
				path.data = rect.slice(0, 6).concat(rect.slice(6), Vector.<Number>([x + (width >> 1), y + height]));
				break;
			case 3:
				rect[0] = rect[6] = rect[8] = rect[0] + arrowSize;
				path.data = rect.slice(0, 8).concat(rect.slice(8), Vector.<Number>([x, y + (height >> 1)]));
				break;
		}
		return path;
	}
	...
	/** 네모 상자의 path 를 만든다 */
	public static function rect(x : Number, y : Number, width : Number, height : Number, path : GraphicsPath = null) : GraphicsPath {
		if (!path) path = new GraphicsPath();
	...
	private static const TAIL_BOX_COMMAND : Vector.<int> = Vector.<int>([1,2,2,2,2,2,2,2]);

	/** 말풍선 형태의 상자를 만든다 */
	public static function tailBox(x : Number, y : Number, width : Number, height : Number, tp : Point, ts : int, path : GraphicsPath = null) : GraphicsPath {
		var xcase : int;
		var ycase : int;
		if (tp.x < x) {
			xcase = 0;
		...
		if (draw < 1) return rect(x, y, width, height);
		...
		path.data.push(x, y);
		if (xcase == 1 && ycase == 0) {
			if (tp.x < (ts >> 1) + x) tp.x = (ts >> 1) + x;
			if (tp.x > x + width - (ts >> 1)) tp.x = x + width - (ts >> 1);
			path.data.push(tp.x - (ts >> 1), y, tp.x, tp.y, tp.x + (ts >> 1), y, x + width, y);
		} else {
			path.data.push(x + width, y);
		}
		if (xcase == 2 && ycase == 1) {
			if (tp.y < (ts >> 1) + y) tp.y = (ts >> 1) + y;
			if (tp.y > y + height - (ts >> 1)) tp.y = y + height - (ts >> 1);
	...
	/** 둥근 모서리를 가진 상자를 만든다 */
	public static function roundRect(x : Number, y : Number, width : Number, height : Number, ellipseWidth : Number, ellipseHeight : Number, path : GraphicsPath = null) : GraphicsPath {
		if (!path) path = new GraphicsPath();
	...
	public static function distort(tlX : Number, tlY : Number, trX : Number, trY : Number, dlX : Number, dlY : Number, drX : Number, drY : Number, path : GraphicsTrianglePath = null) : GraphicsTrianglePath {
		if (!path) path = new GraphicsTrianglePath();
	...
	/** 도넛 형태의 path 를 만든다 */
	public static function donut(x : Number, y : Number, radius : Number, innerRadius : Number, startDeg : Number, endDeg : Number, path : GraphicsPath = null) : GraphicsPath {
		if (!path) path = new GraphicsPath();
	...
	/** 간단한 line path 를 만든다 */
	public static function line(x0 : Number, y0 : Number, x1 : Number, y1 : Number, path : GraphicsPath = null) : GraphicsPath {
		if (!path) path = new GraphicsPath();
